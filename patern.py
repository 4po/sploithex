...
def show_help():
    ...
    print (
        "\Retourne le décalage pour l'argument fourni <str>. Doit être au moins"
          "trois caractères pour une correspondance non ambiguë. Peut être une valeur hexagonale. La Conversion"
              "se fait automatiquement pour les petites architectures endiennes  (ie : x86)"
    Cette "
             " Signifie essentiellement que la châine obtenue par conversion hexagonale est "
                "inversé". ATTENTION : Le décodage des haxagones est sauté pour les valeurs hexagonales valides"
" qui font partie de la mémoire tampon elle-même car le décodage des hexagones est une solution de repli"
       "mesure. Utilisez toujours le préfixe 0x afin de forcer le décodage de l'héxagone."
       "" + os.linesep + ""
       "Pour les motifs de plus de 20280 caractères, vous devez fournir le caractère optionnel"
       "taille de l'argument afin de recevoir toutes les compensations pour le modèle Aa0".)
